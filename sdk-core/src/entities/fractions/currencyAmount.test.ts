...
describe('CurrencyAmount', () => {
  ...
  describe('constructor', () => {
    it('works', () => {
      const token = new Token(ChainId.FXCORE, ADDRESS_ONE, 18)
  ...
  describe('#ether', () => {
    it('produces ether amount', () => {
      const amount = CurrencyAmount.ether(ChainId.FXCORE,100)
      expect(amount.quotient).toEqual(JSBI.BigInt(100))
  ...
  it('token amount can be max uint256', () => {
    const amount = CurrencyAmount.fromRawAmount(new Token(ChainId.FXCORE, ADDRESS_ONE, 18), MaxUint256)
    expect(amount.quotient).toEqual(MaxUint256)
  })
  it('token amount cannot exceed max uint256', () => {
    expect(() =>
      CurrencyAmount.fromRawAmount(new Token(ChainId.FXCORE, ADDRESS_ONE, 18), JSBI.add(MaxUint256, JSBI.BigInt(1)))
    ).toThrow('AMOUNT')
  })
  it('token amount quotient cannot exceed max uint256', () => {
    expect(() =>
      CurrencyAmount.fromFractionalAmount(
        new Token(ChainId.FXCORE, ADDRESS_ONE, 18),
  ...
  describe('#toFixed', () => {
    it('throws for decimals > currency.decimals', () => {
      const token = new Token(ChainId.FXCORE, ADDRESS_ONE, 0)
