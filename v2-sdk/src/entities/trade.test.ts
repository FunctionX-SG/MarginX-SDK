...
describe('Trade', () => {
  ...
  const token3 = new Token(ChainId.FXCORE, '0x0000000000000000000000000000000000000004', 18, 't3')
  ...
  const pair_weth_0 = new Pair(
    CurrencyAmount.fromRawAmount(WETH9[ChainId.FXCORE], JSBI.BigInt(1000)),
    CurrencyAmount.fromRawAmount(token0, JSBI.BigInt(1000))
  )
  ...
  describe('#bestTradeExactIn', () => {
    ...
    it('works for ETHER currency input', () => {
      ...
      expect(result[0].route.path).toEqual([WETH9[ChainId.FXCORE], token0, token1, token3])
      expect(result[0].outputAmount.currency).toEqual(token3)
      expect(result[1].inputAmount.currency).toEqual(ETHER)
      expect(result[1].route.path).toEqual([WETH9[ChainId.FXCORE], token0, token3])
    ...
    it('works for ETHER currency output', () => {
      ...
      expect(result[0].route.path).toEqual([token3, token0, WETH9[ChainId.FXCORE]])
      expect(result[0].outputAmount.currency).toEqual(ETHER)
      expect(result[1].inputAmount.currency).toEqual(token3)
      expect(result[1].route.path).toEqual([token3, token1, token0, WETH9[ChainId.FXCORE]])
